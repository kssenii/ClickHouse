    ...
    for (size_t try_idx = 0; try_idx < max_tries; ++try_idx)
    {
        for (auto & priority : replicas_with_priority)
        {
            ...
            for (size_t i = 0; i < replicas.size(); ++i)
            {
                ...
                auto new_connection = [&]()
                {
                    auto connection = std::make_unique<pqxx::connection>(replica.connection_string);
                ...
                try
                {
                    connection_available = replica.pool->tryBorrowObject(connection, new_connection, pool_wait_timeout);
                }
