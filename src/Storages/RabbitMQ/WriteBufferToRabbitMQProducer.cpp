 ...
 *
 * So currently implemented async batch publishing, but not very fair: event loop is started after every published batch, but it does not
 * stop all publishing untill exactly everything is confirmed - there will actually be two loop runs and then processing will be continued
 * (but eventually it will wait for all confirmations). And also there is no manual republishing now.
*/
