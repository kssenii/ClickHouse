...
void StorageRabbitMQ::initExchange()
{
    ...
    .onError([&](const char * message)
    {
        /* This error be a result of same reasons as above for exchange_name, i.e. it will mean that sharding exchange name appeared to
         * be the same as some other exchange (which purpose is not for sharding). So actual error reason: queue_base parameter is bad.
         */
        throw Exception("Unable to declare exchange. Reason: " + std::string(message), ErrorCodes::LOGICAL_ERROR);
    });
...
void StorageRabbitMQ::shutdown()
{
    ...
    {
        /* Needed because otherwise might get a deadlock, when deactivate streaming task and then inside streaming task try to
         * deactivate some other task
         */
        std::lock_guard lock(task_mutex);

        LOG_TRACE(log, "Deactivating looping thread");
        event_handler->updateLoopState(Loop::STOP);
        looping_task->deactivate();

        LOG_TRACE(log, "Deactivating heartbeating thread");
        heartbeat_task->deactivate();

        LOG_TRACE(log, "Deactivating streaming thread");
        streaming_task->deactivate();
    }
    ...
    LOG_TRACE(log, "Closing connection");
    connection->close();
...
bool StorageRabbitMQ::streamToViews()
{
     ...
     */
    if (event_handler->loopRunning() && task_mutex.try_lock())
    {
        event_handler->updateLoopState(Loop::STOP);
        looping_task->deactivate();
        task_mutex.unlock();
    ...
    if (!event_handler->connectionRunning())
    {
        ...
        {
            /// Reschedule if unable to connect to rabbitmq or return to cancel
            return false;
        }
    }
    else
    {
        ...
        for (auto & stream : streams)
        {
             ...
             *    Also in this case it is ok if failed to send ack, because the next attempt to send ack on the same channel will also
             *    commit all previously not-committed messages. Anyway I do not think that for ack frame this will ever happen.
             */
